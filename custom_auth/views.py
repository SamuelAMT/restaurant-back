...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            LoginLog.objects.create(account=user, ip_address=ip_address, action="login")
...
def logout_view(request):
    if request.method == 'POST':
        auth_logout(request)
        ip_address = request.META.get('REMOTE_ADDR', '')
        LoginLog.objects.create(account=None, ip_address=ip_address, action="logout")
...
#            account=user, session_key=session_key, ip_address = request.META.get('REMOTE_ADDR', '')
#        )
